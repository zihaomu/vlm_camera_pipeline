...
![VLM Camera Web 主页面](doc/assets/vlm-camera-web-dashboard.png)
...
推荐的组合 Demo 使用 Hybrid 同帧引导：YOLO26 持续推理，每 3 秒把同一 Camera frame 的
GPU 编号框和有界 COCO 提示交给 Qwen3-VL，摄像头像素和模型 I/O 不落到 CPU：
...
Prompt-only 网页版使用同一严格 Hybrid 推理链。网页主体是可缩放的实时视频；彩色 YOLO 框与
COCO class name 由 HIP 直接烧入编码前的 YUYV GPU DMA-BUF，不再依赖低频 Canvas overlay。
视频下方固定显示 VLM 字幕，侧栏只读展示 Camera/YOLO/VLM 速度。页面唯一可修改项是 VLM
prompt，保存后从下一次可调度的 3 秒请求生效：

```bash
./scripts/run_yolo_vlm_web_zerocopy.sh
```

默认自动打开 `http://127.0.0.1:8765/`。浏览器流使用
`HIP YUYV DMA-BUF + in-place box/class burn-in -> VAAPI VPP -> VAAPI H.264 -> fMP4/MSE`；
CPU 只转发压缩码流和 prompt/性能状态，不读取摄像头像素或逐帧检测结果。实现边界与短测证据见
[Web Prompt Demo 设计与实施记录](doc/web_prompt_demo_design.md)。

网页播放器现在使用约 `0.16 s` 的低延迟实时缓冲：VAAPI encoder 使用无 B frame 的 CBR
低延迟路径，keyframe 间隔 6 帧，服务端每约 50 ms 发送一个完整 `moof+mdat` 分片；新连接仍
等待 H.264 sync sample。Firefox 40 秒稳态实测 Camera capture→display 中位数约 `156 ms`、
P95 约 `196 ms`，1192 帧内没有新增解码丢帧、确认 rebuffer 或 append queue 溢出。

网页入口默认把 `/dev/video0` 当作前置摄像头，在 NV12→RGB/YUYV 的 HIP kernel 内融合一次水平
去镜像；YOLO、VLM、彩色框和编码视频从源头共用同一物理视角，不会靠 CSS 翻转而把 class name
反写。若接入后置/外置摄像头且设备输出本来就是物理视角，可用
`./scripts/run_yolo_vlm_web_zerocopy.sh --no-camera-horizontal-flip` 关闭。

Hybrid 入口已锁定 `--vlm-interval 3.0`、`--hybrid-max-hints 8` 与严格 zero-copy；原 clean
组合路径继续独立保留，便于回归和 A/B：
...
Hybrid 同帧调度、GPU 编号框、语义对抗和验收边界见
[Hybrid YOLO+VLM 设计与实施日志](doc/hybrid_yolo_vlm_design.md)。
...
Python 环境只由 `uv` 管理，`.venv`、依赖 cache 和应用配置均位于本仓库内。网页的 VAAPI
实时编码已落地；文件录制仍属于独立里程碑。
...
Hybrid 推荐组合入口及当前验收结果见
[doc/hybrid_yolo_vlm_design.md](doc/hybrid_yolo_vlm_design.md)。
Prompt-only Web 前端、GPU 视频编码路径和当前短测结果见
[doc/web_prompt_demo_design.md](doc/web_prompt_demo_design.md)。
